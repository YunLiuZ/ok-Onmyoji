...
class ExplorationTask(BaseBattleTask):
    ...
    def Exploration_page(self):   
        ...
        if not (text:=self.ocr_and_click(['困难'],1,box=self.box_of_screen(0.01, 0.25, 0.13, 0.51))):
            print(text)
            self.log_info('找不到困难')
        ...
        if not (text:=self.ocr_and_click(['探索'],1,box=self.box_of_screen(0.68, 0.79, 0.93, 0.94))):
            print(text)
            self.log_info('找不到探索')
    def Leader_page(self):
        ...
        if text:=self.ocr_and_click('创建',box=self.box_of_screen(0.3457, 0.5653, 0.5074, 0.6243)):
            print(text)
            self.click_relative(0.68,0.80,after_sleep=1)
            return True
